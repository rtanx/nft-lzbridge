...
import { NFTState } from "./State.sol";
...
contract WrappedERC721Implementation is NFTState, Context, IERC721, IERC721Metadata, ERC165 {
    using Address for address;
    using Strings for uint256;

    function initialize(string memory _name, string memory _symbol, address _owner, uint16 _chainId, bytes32 _nativeContract) public initializer {
        _state.name = _name;
    ...
    function balanceOf(address owner_) public view override returns (uint256) {
        require(owner_ != address(0x0), "ERC721: balance query for the zero address");
        return _state.balances[owner_];
    }

    function ownerOf(uint256 tokenId) public view override returns (address) {
        address owner_ = _state.owners[tokenId];
        require(owner_ != address(0x0), "ERC721: owner query for nonexistent token");
        return owner_;
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner_ = WrappedERC721Implementation.ownerOf(tokenId);
    ...
    function _burn(uint256 tokenId) internal {
        require(_exists(tokenId), "Trying to burn non nonexistent tokenId");
    ...
    function _transfer(address from, address to, uint256 tokenId) internal {
        require(WrappedERC721Implementation.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal {
        ...
        emit Approval(WrappedERC721Implementation.ownerOf(tokenId), to, tokenId);
    }
